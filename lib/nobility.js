...
function createNobilityStore({ dataDir, log = console }) {
  ...
  async function resetProfiles(tiers = NOBILITY_TIERS, now = Date.now()) {
    const board = await ensureLoaded();
    const ladder = normalizeNobilityTiers(tiers);
    let reset = 0;
    for (const [userId, entry] of board.entries()) {
      board.set(userId, normalizeProfile({
        ...entry,
        totalXp: 0,
        lastXpAt: 0,
        lastRewardAt: 0,
        lastRewardReason: "scheduled nobility reset",
        updatedAt: new Date(now).toISOString()
      }, ladder));
      reset += 1;
    }
    if (reset) await enqueuePersist();
    return reset;
  }
